...
/// Host-owned disk reservation held until a spill file and all of its clones are deleted.
/// LAKEDAY PATCH: grants physical space before the query starts writing.
pub trait SpillFileGuard: Send + Sync + std::fmt::Debug {}

/// Reclaims disposable host cache space before creating query spill files.
pub trait SpillFileObserver: Send + Sync + std::fmt::Debug {
    /// Return a reservation only after physical reclamation succeeds.
...
pub struct DiskManagerBuilder {
    ...
    spill_file_observer: Option<Arc<dyn SpillFileObserver>>,
}
...
impl Default for DiskManagerBuilder {
    fn default() -> Self {
        Self {
            ...
            spill_file_observer: None,
        }
    }
}

impl DiskManagerBuilder {
    /// Attach the host's reclamation policy to every temporary file and clone.
    pub fn with_spill_file_observer(mut self, observer: Arc<dyn SpillFileObserver>) -> Self {
        self.spill_file_observer = Some(observer);
        self
    }
    ...
    pub fn build(self) -> Result<DiskManager> {
        match self.mode {
            DiskManagerMode::OsTmpDirectory => Ok(DiskManager {
                ...
                spill_file_observer: self.spill_file_observer.clone(),
            }),
...
impl DiskManager {
    ...
    pub fn create_tmp_file(
        ...
        let spill_guard = self
            .spill_file_observer
            .as_ref()
            .map(|observer| observer.before_create())
            .transpose()?;
...
pub struct RefCountedTempFile {
    ...
    // Drop after tempfile: cache growth must not race deletion of the spill file.
    spill_guard: Option<Arc<dyn SpillFileGuard>>,
}
...
impl Clone for RefCountedTempFile {
    fn clone(&self) -> Self {
        Self {
            ...
            spill_guard: self.spill_guard.clone(),
        }
    }
}
