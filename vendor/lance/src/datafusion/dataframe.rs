...
pub struct LanceTableProvider {
    ...
    scan_buffers: Option<(u64, usize, usize)>,
}
...
impl LanceTableProvider {
    ...
    /// Bound read-ahead and decode fan-out under the host's shared query allocation.
    /// The caller reserves these buffers in its execution memory pool.
    pub fn with_scan_buffers(
        mut self,
        io_bytes: u64,
        decode_batches: usize,
        fragments: usize,
    ) -> Self {
        self.scan_buffers = Some((io_bytes, decode_batches, fragments));
        self
    }
...
impl TableProvider for LanceTableProvider {
    ...
    async fn scan(
        ...
        if let Some((io_bytes, decode_batches, fragments)) = self.scan_buffers {
            scan.io_buffer_size(io_bytes)
                .batch_readahead(decode_batches)
