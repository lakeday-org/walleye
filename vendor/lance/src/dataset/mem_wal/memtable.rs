...
impl MemTable {
    ...
    pub async fn insert(&mut self, batch: RecordBatch) -> Result<usize> {
        ...
        // Top-level schema metadata is producer-owned and may differ between
        // shards (Lakeday carries the authenticated `owner_do_id` there).
        // It does not change the columns a MemTable stores, so compare the
        // field definitions while retaining the batch's metadata in the
        // BatchStore for WAL encoding.
    ...
    pub async fn insert_batch_only(&mut self, batch: RecordBatch) -> Result<(usize, u64, usize)> {
        // Validate schema compatibility
        // Top-level schema metadata is not part of the MemTable's column
        // contract; see `insert` for why it must remain on the stored batch.
        if batch.schema().fields() != self.schema.fields() {
            return Err(Error::invalid_input(
                "Batch schema doesn't match MemTable schema",
            ));
    ...
    pub async fn insert_batches_only(
        ...
        for (i, batch) in batches.iter().enumerate() {
            // Top-level schema metadata is intentionally ignored here.  The
            // owner identity is carried in that metadata until the Bitr WAL
            // backend validates and persists the batch.
            if batch.schema().fields() != self.schema.fields() {
