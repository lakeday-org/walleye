...
/// A serialized write supplied to a WAL backend.
///
/// The payload is a complete Arrow IPC stream. Backends must persist it at the
/// requested shard and entry position, or return [`WalWriteResult::AlreadyExists`]
/// when another writer already owns that position.
#[derive(Debug, Clone)]
pub struct WalWrite {
    /// MemWAL shard that owns this entry.
    pub shard_id: Uuid,
    /// Epoch of the writer that produced the entry.
    pub writer_epoch: u64,
    /// One-based MemWAL entry position.
    pub entry_position: u64,
    /// Arrow IPC bytes, including the writer epoch metadata.
    pub payload: Bytes,
}

/// Durable receipt returned by a [`WalBackend`].
///
/// `knowledge` is backend-defined opaque commit material. Lakeday uses it for
/// the replica commit certificate/knowledge; the object-store backend leaves it
/// empty. Lance carries it through the durable `put` result without depending
/// on a particular durability service.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct WalCommit {
    /// MemWAL entry position acknowledged by the backend.
    pub entry_position: u64,
    /// Opaque backend commit knowledge associated with this acknowledgement.
    pub knowledge: Bytes,
}

/// Outcome of an atomic WAL backend write.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum WalWriteResult {
    /// The backend durably accepted the entry.
    Committed(WalCommit),
    /// The requested position already contains an entry.
    AlreadyExists,
}

/// Storage boundary for MemWAL entries.
///
/// Implementations own the durable log and may place it anywhere. Lance still
/// owns the Arrow IPC payload and entry ordering; the backend owns persistence,
/// reads, position discovery, and predecessor fencing. Shard manifests remain
/// in Lance's object store in this seam.
#[async_trait]
pub trait WalBackend: Debug + Send + Sync {
    /// Atomically append one serialized WAL entry.
    async fn append(&self, write: WalWrite) -> Result<WalWriteResult>;

    /// Read one serialized WAL entry, returning `None` when it is absent.
    async fn read(&self, shard_id: Uuid, entry_position: u64) -> Result<Option<Bytes>>;

    /// Return the next one-based entry position for a shard.
    async fn next_position(&self, shard_id: Uuid, hint: Option<u64>) -> Result<u64>;

    /// Persist the data-less predecessor-fence sentinel.
    async fn fence(&self, write: WalWrite) -> Result<WalWriteResult> {
        self.append(write).await
    }

    /// Notification after Lance durably publishes a manifest covering this WAL
    /// prefix. Backends can release local replay buffers through this position.
    /// This is not a request to delete the durable log or alter append cursors.
    async fn checkpointed(&self, _shard_id: Uuid, _covered_position: u64) {}

    /// Return the first entry position retained by this shard's log.
    async fn first_position(&self, shard_id: Uuid) -> Result<u64>;
}
...
pub struct WriterCursors {
    durable: AtomicUsize,
    /// Commit receipts keyed by the writer-global batch watermark they cover.
    /// A single backend append may cover several puts, so a waiter selects the
    /// first receipt whose watermark reaches its target.
    commits: StdMutex<BTreeMap<usize, WalCommit>>,
...
impl WriterCursors {
    pub fn new(durable_write: bool) -> Self {
        ...
        Self {
            ...
            commits: StdMutex::new(BTreeMap::new()),
            notify_tx,
            notify_rx,
    ...
    /// Advance durability and retain the backend receipt for waiters covering
    /// this writer-global batch watermark.
    pub(crate) fn advance_durable_with_commit(&self, global_count: usize, commit: WalCommit) {
        self.durable.fetch_max(global_count, Ordering::AcqRel);
        self.commits
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .insert(global_count, commit);
        self.wake();
    }

    /// Return the first backend receipt whose durable watermark covers a target
    /// batch position.
    fn commit_for(&self, target: usize) -> Option<WalCommit> {
        self.commits
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .range(target..)
...
impl BatchDurableWatcher {
    ...
    /// Return the backend receipt that made this durable target visible.
    ///
    /// Call after [`Self::wait`] or [`Self::wait_for_commit`] has completed.
    pub fn commit(&self) -> Option<WalCommit> {
        self.cursors.commit_for(self.target_durable)
    }

    /// Wait until visible and return the backend receipt that made the target
    /// durable. The receipt is present for every successful durable WAL append,
    /// including the default object-store backend (whose knowledge is empty).
    pub async fn wait_for_commit(&mut self) -> Result<WalCommit> {
        self.wait().await?;
        self.commit()
            .ok_or_else(|| Error::internal("durable WAL wait completed without a commit receipt"))
    }
}
...
pub struct WalFlushResult {
    ...
    pub wal_bytes: usize,
    /// Backend receipt for the appended WAL entry, when one was written.
    pub commit: Option<WalCommit>,
}
...
impl WalFlusher {
    ...
    async fn flush_from_batch_store(
        ...
        self.cursors.advance_durable_with_commit(
            batch_store.global_offset() + end_batch_position,
            append_result.commit.clone(),
        );
        ...
        Ok(WalFlushResult {
            ...
            commit: Some(append_result.commit),
        })
    }
...
/// Default MemWAL backend that stores Arrow IPC entries in Lance's object
/// store. This keeps the upstream behavior unchanged when no custom backend is
/// configured on [`ShardWriterConfig`](super::write::ShardWriterConfig).
#[derive(Debug)]
pub(super) struct ObjectStoreWalBackend {
    object_store: Arc<ObjectStore>,
    base_path: Path,
}

impl ObjectStoreWalBackend {
    /// Create the object-store implementation for one Lance table root.
    pub(super) fn new(object_store: Arc<ObjectStore>, base_path: Path) -> Self {
        Self {
            object_store,
            base_path,
        }
    ...
    /// Resolve a shard's object-store WAL directory.
    fn wal_dir(&self, shard_id: &Uuid) -> Path {
        shard_wal_path(&self.base_path, shard_id)
    }
}

#[async_trait]
impl WalBackend for ObjectStoreWalBackend {
    /// Atomically create the requested object-store WAL object.
    async fn append(&self, write: WalWrite) -> Result<WalWriteResult> {
        let wal_dir = self.wal_dir(&write.shard_id);
        match atomic_put(
            self.object_store.as_ref(),
            &wal_dir,
            &wal_entry_filename(write.entry_position),
            write.payload,
        )
        .await
        {
            Ok(()) => Ok(WalWriteResult::Committed(WalCommit {
                entry_position: write.entry_position,
                knowledge: Bytes::new(),
            })),
            Err(AtomicPutError::AlreadyExists) => Ok(WalWriteResult::AlreadyExists),
            Err(AtomicPutError::Other(error)) => Err(error),
        }
    }

    /// Read one Arrow IPC WAL object from the object store.
    async fn read(&self, shard_id: Uuid, entry_position: u64) -> Result<Option<Bytes>> {
        let path = self
            .wal_dir(&shard_id)
        ...
        let data = match self.object_store.inner.get(&path).await {
            ...
            Err(error) => {
                return Err(Error::io(format!(
                    ...
                    entry_position, shard_id, error
                )));
            }
        };
        data.bytes().await.map(Some).map_err(|error| {
            Error::io(format!(
    ...
    /// Probe or list the object-store WAL to find its next entry position.
    async fn next_position(&self, shard_id: Uuid, hint: Option<u64>) -> Result<u64> {
        let wal_dir = self.wal_dir(&shard_id);
        if let Some(hint) = hint
            && let Some(position) =
                probe_forward_from(self.object_store.as_ref(), &wal_dir, shard_id, hint).await?
        {
            return Ok(position);
        }
        scan_next_position(self.object_store.as_ref(), &wal_dir, shard_id).await
    }

    /// Persist a fence sentinel using the same atomic object-store operation as
    /// a data entry. The appender handles position conflicts and retries.
    async fn fence(&self, write: WalWrite) -> Result<WalWriteResult> {
        self.append(write).await
    }

    /// Find the first retained object-store WAL entry.
    async fn first_position(&self, shard_id: Uuid) -> Result<u64> {
        scan_first_position(
            self.object_store.as_ref(),
...
pub struct WalAppendResult {
    ...
    pub num_rows: usize,
    pub wal_bytes: usize,
    /// Backend receipt for the durable append.
...
impl WalAppender {
    ...
    pub(crate) fn with_claimed_epoch(
        ...
        let backend = Arc::new(ObjectStoreWalBackend::new(object_store, base_path.clone()));
        Self::with_claimed_epoch_and_backend(
            backend,
            base_path,
            shard_id,
            manifest_store,
            writer_epoch,
            next_entry_position_hint_seed,
            retry,
        )
    }

    /// Create an appender around a caller-owned backend after the shard epoch
    /// has already been claimed.
    pub(crate) fn with_claimed_epoch_and_backend(
        backend: Arc<dyn WalBackend>,
        base_path: Path,
        shard_id: Uuid,
        manifest_store: Arc<ShardManifestStore>,
        ...
        loop {
            ...
            match self
                ...
                .append(WalWrite {
                    ...
                    entry_position: pos,
                    payload: wal_data.clone(),
                })
                .await
            {
                Ok(WalWriteResult::Committed(commit)) => {
        ...
        loop {
            match self
                ...
                .fence(WalWrite {
                    ...
                    writer_epoch: self.writer_epoch,
                    entry_position: pos,
                    payload: sentinel.clone(),
                })
                .await
            {
                Ok(WalWriteResult::Committed(_commit)) => {
                    let next = pos.checked_add(1).ok_or_else(|| {
                        Error::io(format!("WAL position overflow for shard {}", self.shard_id))
                    })?;
                    *next_pos = Some(next);
    ...
    async fn discover_next_position(&self) -> Result<u64> {
        let hint = self
            ...
            .ok()
            .flatten()
            .map(|manifest| manifest.wal_entry_position_last_seen);
        self.backend.next_position(self.shard_id, hint).await
    }
}
...
impl WalTailer {
    ...
    pub fn new(object_store: Arc<ObjectStore>, base_path: Path, shard_id: Uuid) -> Self {
        ...
        let backend = Arc::new(ObjectStoreWalBackend::new(object_store, base_path));
        Self::new_with_backend(backend, manifest_store, shard_id)
    }

    /// Create a WAL tailer using a caller-owned backend and Lance manifest
    /// store. The backend supplies entry bytes and position discovery.
    ...
    pub async fn read_entry(&self, entry_position: u64) -> Result<Option<WalReadEntry>> {
        let entry = self.read_entry_without_cursor(entry_position).await?;
        if entry.is_some() {
    ...
    /// Read a WAL entry at the given position without touching the manifest
    /// cursor hint. Replay walks every entry since the last flush — thousands
    /// on a long-unflushed shard — and each hint update is a manifest read plus
    /// a conditional write, so a bulk reader reads with this and records the
    /// tip once through [`Self::record_cursor`] instead.
    pub(crate) async fn read_entry_without_cursor(
        &self,
        entry_position: u64,
    ) -> Result<Option<WalReadEntry>> {
        let Some(bytes) = self.backend.read(self.shard_id, entry_position).await? else {
            return Ok(None);
        };
    ...
    /// Record `entry_position` as the manifest's best-effort cursor hint,
    /// awaiting the single conditional manifest write. Losing the write to a
    /// concurrent manifest commit is fine: the hint only shortens a later
    /// position discovery and is never a correctness input.
    pub(crate) async fn record_cursor(&self, entry_position: u64) {
        best_effort_cursor_update(&self.manifest_store, entry_position).await;
    }

    /// Find the next append position (one past the latest entry).
    pub async fn next_position(&self) -> Result<u64> {
        self.backend
            .next_position(self.shard_id, self.manifest_cursor_hint().await)
            .await
    }

    /// Find the earliest listed WAL position.
    pub async fn first_position(&self) -> Result<u64> {
        self.backend.first_position(self.shard_id).await
    }
