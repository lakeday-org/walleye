...
use super::wal::{
    BatchDurableWatcher, TriggerIndexApply, TriggerWalFlush, WalAppender, WalBackend,
    WalFlushSource, WalOnlyState, WalRetryConfig, WalTailer, WriterCursors, apply_index_range,
    empty_flush_result,
};
...
pub struct ShardWriterConfig {
    ...
    /// Optional backend for durable WAL entries. When `None`, Lance writes
    /// entries to the object store using its standard Arrow IPC layout.
    pub wal_backend: Option<Arc<dyn WalBackend>>,
...
impl ShardWriterConfig {
    ...
    /// Set the durable WAL backend used by this shard writer.
    pub fn with_wal_backend(mut self, backend: Arc<dyn WalBackend>) -> Self {
...
pub struct WriteResult {
    ...
    /// Durable backend receipt when the write waited for a WAL append.
    pub commit: Option<super::wal::WalCommit>,
}
...
async fn replay_memtable_from_wal(
    ...
    // Reuse the writer's manifest store rather than building a second one
    // with the minimum scan batch: a cold open's cursor writes and
    // `read_latest` probes should use the caller's configured parallelism.
    let tailer = WalTailer::new_with_backend(wal_backend, manifest_store, shard_id);
    let mut position = start_position;

    let mut active = make_memtable(base_generation, 0)?;

    loop {
        // Read without the per-entry cursor hint: an unflushed history of N
        // entries would otherwise spawn N detached manifest read+write pairs
        // (measured at ~10 object-store requests per entry) that outlive the
        // open. The tip is recorded once after the loop.
        match tailer.read_entry_without_cursor(position).await? {
    ...
    // One cursor hint for the whole replay: the last entry actually read.
    if position > start_position {
        tailer.record_cursor(position.saturating_sub(1)).await;
    }
...
fn ensure_tombstone_column(
    ...
    // Relabeling to the storage schema must not discard producer metadata.
    // Lakeday carries the authenticated owner identity here so its custom WAL
    // backend can enforce cross-DO isolation after Lance injects tombstones.
    let storage_schema = Arc::new(ArrowSchema::new_with_metadata(
        storage_schema.fields().clone(),
        batch.schema().metadata().clone(),
    ));
    RecordBatch::try_new(storage_schema, columns).map_err(|e| {
...
impl ShardWriter {
    ...
    pub async fn open(
        ...
        let position_hint_seed = manifest
            ...
            .saturating_add(1);
        let wal_backend: Arc<dyn WalBackend> = config.wal_backend.clone().map_or_else(
            || {
                Arc::new(super::wal::ObjectStoreWalBackend::new(
                    object_store.clone(),
                    base_path.clone(),
                )) as Arc<dyn WalBackend>
            },
            |backend| backend,
        );
        // The persisted manifest is the authority for which WAL entries are
        // already represented by Lance generations, including on a reused backend.
        wal_backend
            .checkpointed(shard_id, manifest.replay_after_wal_entry_position)
            .await;
        let wal_appender = Arc::new(WalAppender::with_claimed_epoch_and_backend(
            wal_backend.clone(),
            base_path.clone(),
            shard_id,
    ...
    pub async fn delete(&self, keys: Vec<RecordBatch>) -> Result<WriteResult> {
        ...
        let commit = if let Some(mut watcher) = watcher {
            if self.config.durable_write {
                Some(watcher.wait_for_commit().await?)
            } else {
                watcher.wait().await?;
                None
            }
    ...
    async fn put_wal_only(
        ...
        let commit = if let Some(mut watcher) = durable_watcher {
            Some(watcher.wait_for_commit().await?)
        } else {
            None
        };
    ...
    /// Flush everything the writer holds in memory to Lance generations
    /// and leave it open.
    ///
    /// The stages are [`Self::close`]'s, minus the teardown: drain the
    /// active memtable's index apply, append its unflushed tail to the WAL,
    /// freeze it, then wait for every frozen memtable's flush. Each flush
    /// stamps the manifest's `replay_after_wal_entry_position`, so a later
    /// open — after a graceful close *or* a crash, suspend, or deploy that
    /// never closed the writer — replays only the entries appended after
    /// this call instead of the whole WAL. A host that never reaches the
    /// size-triggered flush (small objects with many tiny entries) calls
    /// this on a cadence to keep its cold-open cost bounded.
    ///
    /// The caller must not interleave puts with the call; Lakeday holds its
    /// event gate across it.
    ///
    /// # Errors
    ///
    /// Returns an error in WAL-only mode, when the writer is fenced or
    /// poisoned, when the WAL tail cannot be persisted, or when a flush
    /// reports failure or its handler exits before reporting.
    #[instrument(name = "sw_checkpoint", level = "info", skip_all, fields(shard_id = %self.config.shard_id, epoch = self.epoch))]
    pub async fn checkpoint(&self) -> Result<()> {
        let WriterMode::MemTable {
            ...
            return Err(Error::invalid_input(
                "checkpoint not available in WAL-only mode (no MemTable)",
            ));
        };
        self.check_fenced().await?;
        self.wal_flusher.check_poisoned()?;

        let (batch_store, indexes, batch_count) = {
            let st = state.read().await;
            (
                st.memtable.batch_store(),
                st.memtable.indexes_arc(),
                st.memtable.batch_count(),
            )
        };

        // Settle the index apply owed to the active memtable before it is
        // flushed; a generation whose indexes never saw its tail has a hole.
        if batch_count > 0
            ...
            && indexes.indexed_count() < batch_count
        {
            ...
            writer_state.trigger_index_apply(Arc::clone(&batch_store), indexes, batch_count)?;
            watcher.wait().await?;
        }

        if batch_count > 0 {
            Self::flush_final_wal(
        ...
        // Freeze the active memtable (if any rows) so it joins the flush
        // queue, then wait for every frozen memtable — including ones a
        // size or interval trigger froze earlier — to land in the manifest.
        ...
        for mut watcher in watchers {
            match watcher.await_value().await {
                ...
                None => {
                    return Err(Error::io(
                        "MemTable flush handler exited before reporting completion during checkpoint",
                    ));
                }
            }
        }
...
mod tests {
    ...
    /// Records custom WAL calls so the backend seam tests observe the complete
    /// append/replay/fence lifecycle without involving the object store WAL.
    #[derive(Debug, Default)]
    struct RecordingWalBackend {
        entries: std::sync::Mutex<std::collections::BTreeMap<(Uuid, u64), bytes::Bytes>>,
        appends: std::sync::Mutex<Vec<super::super::wal::WalWrite>>,
        reads: std::sync::Mutex<Vec<(Uuid, u64)>>,
        fences: std::sync::Mutex<Vec<super::super::wal::WalWrite>>,
    }

    impl RecordingWalBackend {
        fn commit_for(write: &super::super::wal::WalWrite) -> super::super::wal::WalCommit {
            super::super::wal::WalCommit {
                entry_position: write.entry_position,
                knowledge: bytes::Bytes::from_static(b"recording-commit"),
            }
        }

        fn append_inner(
            &self,
            write: super::super::wal::WalWrite,
        ) -> lance_core::Result<super::super::wal::WalWriteResult> {
            let key = (write.shard_id, write.entry_position);
            let mut entries = self.entries.lock().expect("entries mutex poisoned");
            if entries.contains_key(&key) {
                return Ok(super::super::wal::WalWriteResult::AlreadyExists);
            }
            entries.insert(key, write.payload.clone());
            Ok(super::super::wal::WalWriteResult::Committed(
                Self::commit_for(&write),
            ))
        }
    }

    #[async_trait::async_trait]
    impl super::super::wal::WalBackend for RecordingWalBackend {
        async fn append(
            &self,
            write: super::super::wal::WalWrite,
        ) -> lance_core::Result<super::super::wal::WalWriteResult> {
            self.appends
                .lock()
                .expect("appends mutex poisoned")
                .push(write.clone());
            self.append_inner(write)
        }
        ...
        async fn read(
            ...
            shard_id: Uuid,
            entry_position: u64,
        ) -> lance_core::Result<Option<bytes::Bytes>> {
            self.reads
                .lock()
                .expect("reads mutex poisoned")
                .push((shard_id, entry_position));
            Ok(self
                .entries
                .lock()
                .expect("entries mutex poisoned")
                .get(&(shard_id, entry_position))
                .cloned())
        }
        ...
        async fn fence(
            ...
            self.fences
                .lock()
                .expect("fences mutex poisoned")
                .push(write.clone());
            self.append_inner(write)
        }

        async fn first_position(&self, shard_id: Uuid) -> lance_core::Result<u64> {
            Ok(self
                .entries
                .lock()
                .expect("entries mutex poisoned")
    ...
    fn custom_backend_schema() -> Arc<ArrowSchema> {
    ...
    fn custom_backend_batch(schema: &Arc<ArrowSchema>, id: i32) -> RecordBatch {
        RecordBatch::try_new(
            schema.clone(),
            vec![
        ...
        .expect("valid custom backend batch")
    }

    async fn open_custom_backend_writer(
        backend: Arc<RecordingWalBackend>,
    ) -> (ShardWriter, Arc<ObjectStore>, TempDir, Uuid) {
        let (store, base_path, _base_uri, temp_dir) = create_local_store().await;
        let shard_id = Uuid::new_v4();
        let config = ShardWriterConfig::new(shard_id).with_wal_backend(backend);
        let writer = ShardWriter::open(
            store.clone(),
            base_path,
            format!("file://{}", temp_dir.path().display()),
            config,
            custom_backend_schema(),
            vec![],
        )
        .await
        .expect("custom backend writer opens");
        (writer, store, temp_dir, shard_id)
    }

    #[tokio::test]
    async fn test_custom_wal_backend_append_read_and_fence() {
        let backend = Arc::new(RecordingWalBackend::default());
        let (writer, _store, temp_dir, shard_id) =
            open_custom_backend_writer(backend.clone()).await;
        writer
            .put(vec![custom_backend_batch(&custom_backend_schema(), 7)])
            .await
            .expect("custom append succeeds");
        writer.close().await.expect("first writer closes");

        let (second, _store, second_shard_id) =
            open_custom_backend_writer_for_existing(backend.clone(), &temp_dir, shard_id).await;
        assert_eq!(second_shard_id, shard_id);
        second.close().await.expect("successor closes");

        assert_eq!(backend.appends.lock().unwrap().len(), 1);
        assert!(
            !backend.reads.lock().unwrap().is_empty(),
            "reopen must replay through the custom read method"
        );
        assert_eq!(backend.fences.lock().unwrap().len(), 1);
    }

    #[tokio::test]
    async fn test_custom_backend_commit_knowledge_reaches_durable_put() {
        let backend = Arc::new(RecordingWalBackend::default());
        let (writer, _store, _temp_dir, _shard_id) = open_custom_backend_writer(backend).await;
        let result = writer
            .put(vec![custom_backend_batch(&custom_backend_schema(), 11)])
            .await
            .expect("durable custom append succeeds");
        assert_eq!(
            result
                .commit
                .as_ref()
                .map(|commit| commit.knowledge.as_ref()),
            Some(b"recording-commit".as_slice())
        );
        writer.close().await.expect("writer closes");
    }

    #[tokio::test]
    async fn test_custom_backend_does_not_write_object_store_wal() {
        let backend = Arc::new(RecordingWalBackend::default());
        let (writer, _store, temp_dir, shard_id) = open_custom_backend_writer(backend).await;
        writer
            .put(vec![custom_backend_batch(&custom_backend_schema(), 13)])
            .await
            .expect("custom append succeeds");
        writer.close().await.expect("writer closes");
        ...
        assert!(
            !wal_dir.exists(),
            "custom backend must not create stock object-store WAL files"
        );
    }

    async fn open_custom_backend_writer_for_existing(
        backend: Arc<RecordingWalBackend>,
        temp_dir: &TempDir,
        shard_id: Uuid,
    ) -> (ShardWriter, Arc<ObjectStore>, Uuid) {
        // This helper reuses the same filesystem root as the first writer. The
        // backend owns the WAL bytes, while the manifest remains in the root.
        ...
        .await
        .expect("custom backend successor opens");
        (writer, store, shard_id)
    }
    ...
    #[test]
    fn test_ensure_tombstone_column_preserves_input_metadata() {
        let base = create_test_schema();
        let storage = schema_with_tombstone(&base);
        let input = create_test_batch(&base, 0, 1);
        let schema = Arc::new(ArrowSchema::new_with_metadata(
            base.fields().to_vec(),
            std::collections::HashMap::from([(
                "owner_do_id".to_owned(),
                "tenant/default/worker/binding/object".to_owned(),
            )]),
        ));
        let input = RecordBatch::try_new(schema, input.columns().to_vec()).unwrap();

        let output = ensure_tombstone_column(input, &storage).unwrap();
        ...
        assert_eq!(
            output
                ...
                .get("owner_do_id")
                .map(String::as_str),
            Some("tenant/default/worker/binding/object")
        );
    }
