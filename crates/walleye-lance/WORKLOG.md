...
- Fixed WAL payload retention: Lance notifies the backend only after publishing its durable checkpoint, including automatic flushes and startup from an existing manifest. The adapter releases that prefix without changing its append cursor or commit certificate, and historical reads cannot re-cache it. Regression tests cover repeated checkpoints, later writes, reopen, delayed notifications, and automatic flushes.
