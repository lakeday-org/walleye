...
mod tests {
    ...
    fn every_route_has_exactly_one_requirement() {
        ...
        let expected: Vec<(&str, &str, Required)> = vec![
            ...
            ("POST", "/v1/see", Data(Read)),
            ("GET", "/v1/see/tables/{table}", Data(Read)),
            ("GET", "/v1/dashboards", Data(Read)),
            ("GET", "/v1/dashboards/{name}", Data(Read)),
            ("PUT", "/v1/dashboards/{name}", Data(Manage)),
            ("DELETE", "/v1/dashboards/{name}", Data(Manage)),
            ("POST", "/v1/dashboards/{name}/chat", Data(Manage)),
